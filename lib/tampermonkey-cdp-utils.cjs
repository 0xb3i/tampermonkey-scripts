...
async function connectToChromeOverCDP(endpointUrl) {
  return chromium.connectOverCDP(endpointUrl || DEFAULT_CDP_ENDPOINT, { noDefaults: true });
}
...
async function getPrimaryPage(browser) {
  ...
  if (typeof context.newPage !== 'function') {
    throw new Error('No reusable page found in the connected browser, and the default context cannot create a new page.');
  }
...
async function openTampermonkeyPage(page, options) {
  ...
  if (options && options.tabLabel) {
    await page.getByText(String(options.tabLabel), { exact: true }).first().click({ force: true });
    await waitForDocumentReady(page);
  }
}
...
async function clickTampermonkeyConfirm(page) {
  var clicked = await page.evaluate(function () {
    ...
    var target = controls.find(function (el) {
      ...
      return /重新安装|更新用户脚本|更新|确认|覆盖|Reinstall|Install|Update|Confirm|Overwrite/i.test(text) &&
...
async function readTargetEditorState(page, meta) {
  ...
  return page.evaluate(function (payload) {
    ...
    function readCodeMirrorValue(el) {
      if (!el) return '';
      var instance = el.CodeMirror || (typeof window !== 'undefined' && window.CodeMirror && typeof window.CodeMirror.fromTextArea === 'function' ? null : null);
      if (instance && typeof instance.getValue === 'function') {
        return String(instance.getValue() || '');
      }
      var codeText = el.querySelector('.CodeMirror-code');
      if (!codeText) return '';
      return Array.from(codeText.querySelectorAll('pre')).map(function (line) {
        return line.textContent || '';
      }).join('\n');
    }
    var target = codeMirrors.map(function (el) {
      var value = readCodeMirrorValue(el);
      return {
        value: value,
...
async function syncTampermonkeyScript(page, options) {
  ...
  await page.waitForFunction(function (expectedName) {
    var codeMirrors = Array.from(document.querySelectorAll('.CodeMirror'));
    return codeMirrors.some(function (el) {
      var instance = el.CodeMirror;
      if (instance && typeof instance.getValue === 'function') {
        var value = String(instance.getValue() || '');
        if (value.indexOf(String(expectedName || '')) !== -1) return true;
      }
      var text = Array.from(el.querySelectorAll('.CodeMirror-code pre')).map(function (pre) {
        return pre.textContent || '';
      }).join('\n');
      return text.indexOf(String(expectedName || '')) !== -1;
    });
  }, meta.name, { timeout: 15000 });
